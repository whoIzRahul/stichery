...
function validate(name: string, phone: string, email: string, password: string): FieldErrors {
...
function PasswordStrength({ password }: { password: string }) {
  ...
  return (
    <div className="mt-1.5 flex items-center gap-2" aria-live="polite" aria-atomic="true">
      ...
      <span className="min-w-[36px] text-right font-ui text-warm-gray text-xs">{label}</span>
    </div>
  );
}
    ...
    <div className="relative hidden flex-col justify-between overflow-hidden bg-linen px-12 py-14 dark:bg-linen md:flex md:w-[44%] lg:w-[46%]">
      ...
      <div className="relative z-10 max-w-[340px]">
        <blockquote className="font-heading text-4xl text-espresso leading-[1.1] tracking-tight lg:text-[2.8rem]">
          "Every stitch <em className="text-terracotta not-italic">tells</em> a story."
        </blockquote>
        <p className="mt-4 font-body text-sm text-warm-gray leading-relaxed">
          Join thousands of customers who bring handmade warmth into their homes. Each piece crafted
          with care in the heart of Nepal.
        </p>
        ...
        <div className="mt-8 flex items-start gap-8">
          {[
            ...
            <div key={l} className="flex flex-col gap-0.5">
              <span className="font-heading text-2xl text-espresso">{n}</span>
              <span className="font-ui text-warm-gray text-[11px] uppercase tracking-wide">
                {l}
              </span>
            </div>
          ))}
...
export function RegisterForm() {
  ...
  const errors = validate(values.name, values.phone, values.email, values.password);
      ...
      <section className="flex flex-1">
        ...
        <div className="flex flex-1 items-center justify-center px-5 py-16 sm:px-10">
          <div className="w-full max-w-md rounded-lg border border-sand bg-cream p-10 text-center shadow-sm dark:border-sand/30 dark:bg-linen">
            ...
            <h2 className="font-heading text-2xl text-espresso">Account Created!</h2>
            <p className="mt-2 font-body text-sm text-warm-gray">
              Welcome to Stitchery. Please check your email to verify your account.
            </p>
            <Link
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-5 py-10 sm:px-10">
        <div className="w-full max-w-[420px]">
          ...
          <div className="mb-7">
            ...
            <h1 className="mt-3 font-heading text-3xl text-espresso sm:text-4xl">Create account</h1>
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4 mb-5 ">
            {/* Full Name */}
            <FieldWrapper id="name" label="Full Name" error={errors.name} touched={!!touched.name}>
              <input
                id="name"
                type="text"
                name="name"
